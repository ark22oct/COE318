...
//TODO Add instance variables (Hint: you only need 2!)
    ...
    private double correctResVal;
    private double correctVoltVal;
    private double correctCrrntVal;
    
    public Resistor(double resistance) {
        correctResVal = resistance;
...
//FIX THIS so that it returns the actual voltage
        correctVoltVal= correctCrrntVal * correctResVal; //Applying Ohm's Law
        return correctVoltVal;
    }
...
//is between 50 and -50, otherwise print an error message.
        if(voltage >= -50.0 && voltage <= 50.0){
            correctVoltVal = voltage;
        }
        else{
            System.out.println("Error! Values not within specified range of"
                    + " -50.0 to 50.0");
        }
    }
...
//is between 5 and -5, otherwise print an error message.
     ...
     {
         correctCrrntVal = current;
     }  
     else 
     {
         System.out.println("Error! Values not within specified range of -5.0 "
                 + "to 5.0");
     }
    }
